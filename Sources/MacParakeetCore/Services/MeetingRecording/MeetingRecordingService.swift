...
public actor MeetingRecordingService: MeetingRecordingServiceProtocol {
    ...
    private let realtimeTranscriberFactory: @Sendable () -> (any MeetingRealtimeTranscribing)?
    ...
    private var realtimeTranscriber: (any MeetingRealtimeTranscribing)?
    private var realtimeTranscriptTask: Task<Void, Never>?
    ...
    public init(
        ...
        self.init(
            ...
            realtimeTranscriberFactory: {
                SonioxLiveSettings.isEnabled() ? SonioxMeetingTranscriber() : nil
            },
    ...
    init(
        ...
        finalSpeechEngineSelection: @escaping @Sendable () -> SpeechEngineSelection? = { nil },
        isVadLiveChunkingEnabled: @escaping @Sendable () -> Bool = { false },
        realtimeTranscriberFactory: @escaping @Sendable () -> (any MeetingRealtimeTranscribing)? = { nil },
        ...
        self.realtimeTranscriberFactory = realtimeTranscriberFactory
    ...
    public func startRecording(
        ...
        do {
            ...
            await startRealtimeTranscriptionIfConfigured()
            ...
            if realtimeTranscriber == nil, let previewSpeechEngine = session.speechPlan.preview {
    ...
    private func cleanupFailedStart(folderURL: URL) async {
        ...
        await stopRealtimeTranscription(cancelProvider: true)
    ...
    public func stopRecording() async throws -> MeetingRecordingOutput {
        ...
        await liveChunkTranscriber.finishSession()
        await stopRealtimeTranscription(cancelProvider: false)
        await releaseSpeechEngineLease()
    ...
    private func startRealtimeTranscriptionIfConfigured() async {
        guard realtimeTranscriber == nil,
              let transcriber = realtimeTranscriberFactory()
        else { return }
        do {
            try await transcriber.start(vad: await liveVADService())
            realtimeTranscriber = transcriber
            let updates = await transcriber.transcriptUpdates
            realtimeTranscriptTask = Task { [weak self] in
                for await update in updates {
                    guard !Task.isCancelled else { return }
                    await self?.yieldRealtimeTranscriptUpdate(update)
                }
            }
            logger.info("soniox_meeting_transcription_started")
        } catch {
            logger.error(
                "soniox_meeting_transcription_unavailable error_type=\(AudioCaptureDiagnostics.errorType(error), privacy: .public) error_detail=\(error.localizedDescription, privacy: .private)"
            )
            realtimeTranscriber = nil
        }
    }

    private func yieldRealtimeTranscriptUpdate(_ update: MeetingTranscriptUpdate) {
        guard currentSession != nil else { return }
        transcriptContinuation?.yield(update)
    }

    private func stopRealtimeTranscription(cancelProvider: Bool) async {
        guard let transcriber = realtimeTranscriber else { return }
        if !cancelProvider {
            if let folderURL = currentSession?.folderURL,
               let archive = await transcriber.archive() {
                do {
                    try SonioxMeetingTranscriptStore.save(archive, folderURL: folderURL, fileManager: fileManager)
                } catch {
                    logger.error(
                        "soniox_transcript_archive_failed error_type=\(AudioCaptureDiagnostics.errorType(error), privacy: .public)"
                    )
                }
            }
        }
        realtimeTranscriptTask?.cancel()
        realtimeTranscriptTask = nil
    ...
    private func ingestResampledSamples(
        ...
        await realtimeTranscriber?.append(samples, source: source)
    ...
    private func handleCaptureOrchestratorOutput(
        ...
        guard realtimeTranscriber == nil,
              ...
              session.supportsLiveChunkTranscription
        else { return }
