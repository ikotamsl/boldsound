...
public actor TranscriptionService: SpeechEngineOverrideTranscriptionService {
    ...
    private func transcribeMeetingAudio(
        ...
        let sonioxArchive = try? SonioxMeetingTranscriptStore.load(
            folderURL: recording.folderURL,
            fileManager: .default
        )
        let diarizationRequested = sonioxArchive != nil
        ...
        do {
            if let sonioxArchive {
                let finalized = SonioxMeetingTranscriptFinalizer.finalize(sonioxArchive)
                transcription.rawTranscript = finalized.rawTranscript
                transcription.cleanTranscript = nil
                transcription.wordTimestamps = finalized.words.isEmpty ? nil : finalized.words
                transcription.language = finalized.language
                transcription.engine = "soniox"
                transcription.engineVariant = sonioxArchive.model
                transcription.durationMs = max(
                    Int((recording.durationSeconds * 1_000).rounded()),
                    finalized.durationMs ?? 0
                )
                transcription.speakers = finalized.speakers.isEmpty ? nil : finalized.speakers
                transcription.speakerCount = finalized.speakers.isEmpty ? nil : finalized.speakers.count
                transcription.diarizationSegments = finalized.diarizationSegments.isEmpty
                    ? nil
                ...
                return try await completeTranscription(
                    ...
                    diarizationApplied: !finalized.speakers.isEmpty,
                    allowTextRewriting: false
                )
            }
    ...
    private func completeTranscription(
        ...
        persistResult: Bool = true,
        allowTextRewriting: Bool = true
    ) async throws -> Transcription {
        var formatterRun: LLMRun?
        if allowTextRewriting {
            ...
            formatterRun = formatterOutcome.run
            ...
            // Final Soniox tokens are the canonical transcript. Do not run text
            // refinement or an LLM formatter that could add words not spoken.
            transcription.cleanTranscript = nil
        }
        ...
        if persistResult {
            ...
            await llmRunRecorder.record(formatterRun)
        }
