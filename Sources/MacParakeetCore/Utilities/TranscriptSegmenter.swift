...
public enum TranscriptSegmenter {
    ...
    /// Build readable paragraphs for independently timed realtime sources.
    /// Grouping a globally sorted mic/system stream directly would split a
    /// paragraph every time overlapping source timestamps alternate.
    public static func groupParallelSpeakersIntoSegments(
        words: [WordTimestamp]
    ) -> [TranscriptSegment] {
        Dictionary(grouping: words, by: { $0.speakerId ?? "" })
            .values
            .flatMap { speakerWords in
                groupIntoSegments(words: speakerWords.sorted { $0.startMs < $1.startMs })
            }
            .sorted {
