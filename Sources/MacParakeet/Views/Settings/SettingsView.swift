...
struct SettingsView: View {
    ...
    @State private var sonioxEnabled: Bool
    @State private var sonioxAPIKey: String
    @State private var sonioxLanguageHints: String
    ...
    init(
        ...
        self._sonioxEnabled = State(initialValue: SonioxLiveSettings.isEnabled())
        self._sonioxAPIKey = State(initialValue: (try? SonioxCredentialStore().loadAPIKey()) ?? "")
        self._sonioxLanguageHints = State(
            initialValue: SonioxLiveSettings.languageHints().joined(separator: ", ")
        )
    }
    ...
    private var engineTabContent: some View {
        scrollableTabBody {
            ...
            sonioxRealtimeCard.id("engine.soniox")
    ...
    private var sonioxRealtimeCard: some View {
        SettingsCard(
            title: "Soniox Realtime v5",
            subtitle: "Optional cloud live transcription for dictation and meetings.",
            icon: "cloud"
        ) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.sm) {
                Toggle("Use Soniox for live speech", isOn: $sonioxEnabled)
                    .toggleStyle(.switch)
                    .onChange(of: sonioxEnabled) { _, enabled in
                        SonioxLiveSettings.setEnabled(enabled)
                    }

                SecureField("Soniox API key", text: $sonioxAPIKey)
                    .textFieldStyle(.roundedBorder)

                TextField("Language hints, for example: en, ru, tr", text: $sonioxLanguageHints)
                    .textFieldStyle(.roundedBorder)

                Text("Use ISO language codes. Unsupported hints are ignored so automatic language detection can continue.")
                    .font(DesignSystem.Typography.caption)
                    .foregroundStyle(DesignSystem.Colors.textSecondary)

                HStack(spacing: DesignSystem.Spacing.sm) {
                    Button("Save") { saveSonioxSettings() }
                        .buttonStyle(.borderedProminent)
                    Button("Test connection") { testSonioxConnection() }
                        .buttonStyle(.bordered)
                        .disabled(sonioxAPIKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                    Button("Remove key", role: .destructive) { removeSonioxKey() }
                        .buttonStyle(.bordered)
                }
                ...
                Text(
                    "Microphone and system audio use separate sessions with speaker detection on both. Local Silero VAD suppresses confirmed silence and sends a keepalive every 10 seconds. Soniox bills persistent realtime sessions by elapsed connection duration, including silent periods."
                )
                .font(DesignSystem.Typography.caption)
                .foregroundStyle(DesignSystem.Colors.textSecondary)
            }
    ...
    private func saveSonioxSettings() {
        do {
            try SonioxCredentialStore().saveAPIKey(sonioxAPIKey)
            let hints = sonioxLanguageHints.split(separator: ",").map(String.init)
            let unsupportedHints = SonioxLiveSettings.unsupportedLanguageHints(hints)
            SonioxLiveSettings.setLanguageHints(hints)
            sonioxLanguageHints = SonioxLiveSettings.languageHints().joined(separator: ", ")
            SonioxLiveSettings.setEnabled(sonioxEnabled)
            if unsupportedHints.isEmpty {
                sonioxStatus = "Saved securely in macOS Keychain."
            } else {
                sonioxStatus = "Saved. Unsupported Soniox hints ignored: \(unsupportedHints.joined(separator: ", "))."
            }
        } catch {
            sonioxStatus = error.localizedDescription
        }
    }

    private func removeSonioxKey() {
        do {
            try SonioxCredentialStore().deleteAPIKey()
            sonioxAPIKey = ""
            sonioxEnabled = false
            SonioxLiveSettings.setEnabled(false)
            sonioxStatus = "API key removed."
        } catch {
            sonioxStatus = error.localizedDescription
        }
    }
            ...
            do {
                ...
                try await client.connect(
                    configuration: SonioxSessionConfiguration(
                        apiKey: key,
                        languageHints: SonioxLiveSettings.languageHints(),
                        enableSpeakerDiarization: true,
                        enableEndpointDetection: false
                    ),
                    epoch: ConnectionEpoch(source: .microphone, originalStartMs: 0)
                )
                let outcome = await withTaskGroup(of: String.self) { group in
                    group.addTask {
                        for await event in events {
                            if case .failed(_, _, let message, _) = event { return message }
                        }
                        return "Connection closed before validation."
                    }
                    group.addTask {
                        try? await Task.sleep(for: .seconds(2))
                        return "Connected to Soniox realtime v5."
                    }
                    let first = await group.next() ?? "Connection test failed."
                    group.cancelAll()
                    return first
                }
                await client.cancel()
                sonioxStatus = outcome
            } catch {
                await client.cancel()
                sonioxStatus = error.localizedDescription
            }
