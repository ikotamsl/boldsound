...
final class MeetingRecordingFlowCoordinator {
        ...
        -> [MeetingRecordingPreviewLine]
    {
        ...
        let sourceScoped = update.words.contains { $0.speakerId?.contains(":") == true }
        let segments = sourceScoped
            ? TranscriptSegmenter.groupParallelSpeakersIntoSegments(words: update.words)
            : TranscriptSegmenter.groupIntoSegments(words: update.words)
        return segments.map { segment in
            let source = segment.speakerId.flatMap { speakerID -> AudioSource? in
                if let direct = AudioSource(rawValue: speakerID) { return direct }
                guard let prefix = speakerID.split(separator: ":", maxSplits: 1).first else {
                    return nil
                }
                return AudioSource(rawValue: String(prefix))
            }
