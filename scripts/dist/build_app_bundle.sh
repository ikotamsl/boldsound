...
#   BUNDLE_CLI         (default: 0) bundle the upstream CLI
#   BUNDLE_YTDLP       (default: 0) bundle yt-dlp helper seed
#   YTDLP_PATH         (default: auto-download latest) source yt-dlp binary to bundle
#   BUNDLE_NODE        (default: 0) bundle Node runtime for yt-dlp
...
APP_NAME="${APP_NAME:-BoldSound}"
BUNDLE_ID="${BUNDLE_ID:-com.boldsound.prototype}"
...
BUNDLE_CLI="${BUNDLE_CLI:-0}"
if [[ "$BUNDLE_CLI" == "1" ]]; then
  copy_cli_binary
else
  echo "Skipping CLI bundling (BUNDLE_CLI=0)"
fi
...
SU_FEED_URL="${SU_FEED_URL:-https://updates.boldsound.invalid/appcast.xml}"
...
<dict>
  ...
  <string>BoldSound needs microphone access for dictation and meeting speaker detection.</string>
  <key>NSAudioCaptureUsageDescription</key>
  <string>BoldSound needs system audio recording access to transcribe system playback without a virtual audio device.</string>
  <key>NSCalendarsFullAccessUsageDescription</key>
  <string>BoldSound can read your calendar for optional meeting reminders.</string>
  <key>SUFeedURL</key>
