...
BoldSound Soniox meetings keep the lock schema unchanged. Recovery first uses a
valid `soniox-live-transcript.json` final-token sidecar when present; otherwise
it uses the captured/current local Final Transcription route described above.
The sidecar is additive session data, not lock metadata, and an empty finalized
sidecar is a valid no-speech result rather than a reason to invoke local STT.
...
- Preview-engine provenance is intentionally not part of the lock. It is
  additive archived metadata only; recovery uses finalized Soniox tokens when
  present and otherwise needs the authoritative local fallback route.
