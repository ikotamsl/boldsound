...
- `MeetingTranscriptFinalizer` / meeting finalization: completes the DB row and
  final transcript.
- `SonioxMeetingTranscriber`: when Soniox mode is active, gracefully finalizes
  each opened source session and writes the final-token sidecar used by meeting
  finalization.
...
- `meeting-recording-metadata.json`: optional source-alignment and speech-route
  sidecar. `speechEngine` is the authoritative local fallback selection when no
  finalized Soniox sidecar is present;
...
- `soniox-live-transcript.json`: optional Soniox `stt-rt-v5` final-token
  sidecar. Its presence makes Soniox the authoritative transcript source for
  that meeting. Finalization preserves source-qualified mic/system speaker IDs,
  word timings, and speaker phrases; it does not rerun local STT or rewrite the
  transcript with deterministic/LLM text transforms. Explicit non-speech
  annotations and short isolated very-low-confidence noise runs are excluded.
  The sidecar may validly contain no words when VAD observed no speech.
