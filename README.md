<p align="center">
  <img src="Assets/AppIcon-1024x1024.png" width="128" height="128" alt="BoldSound app icon">
</p>

<h1 align="center">BoldSound</h1>

<p align="center">
  Native microphone and system-audio transcription for Mac.
</p>

BoldSound is a macOS speech prototype derived from
[MacParakeet](https://github.com/moona3k/macparakeet) at commit
`c6f9454016b6469d9b9531c5ab05c7129ace9b58`. This derivative remains
GPL-3.0 licensed and retains the upstream copyright and third-party notices.

It adds optional Soniox `stt-rt-v5` live transcription with independent
microphone and ScreenCaptureKit system-audio sessions, per-source local VAD,
and speaker diarization on both inputs. It never installs or requires
BlackHole, Loopback, or another virtual audio device.

To enable the prototype, launch with `scripts/dev/run_app.sh`, grant
Microphone, Screen & System Audio Recording, and Accessibility permissions,
then open Settings → Engine → Soniox Realtime v5. The API key is stored in
macOS Keychain.

Soniox bills persistent realtime sessions by elapsed connection duration even
while VAD suppresses silent audio. In Soniox mode, the server's gracefully
finalized tokens are the authoritative meeting transcript and retain separate
mic/system speaker identities. BoldSound excludes non-speech annotations and
does not run transcript rewriting over those final tokens. Separate local
meeting recordings remain the durable recovery fallback.
